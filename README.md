...
The `Replacement for T# M6` field controls the tool-change command written
for each operation. Use `{tool}` as a placeholder for the actual Fusion tool
number; for example, `T{tool} M6`. Separate multiple output lines with a colon
(`:`). Leave the field blank to remove the included tool-change commands.

By default, the first operation's tool change is omitted. Enable `Include first
tool change` when the controller must also process the initial tool. This is
based on operation order, not the tool number, so it works when the first tool
is T2 or any other number.
